...
function ComponentManager({ config }) {
  ...
  const [showChangeAddress, setShowChangeAddress] = useState(config.changeAddress);
  const [showDataCard, setShowDataCard] = useState(config.dataCard);
  const [showResetPasswordCard, setShowResetPasswordCard] = useState(config.resetPasswordCard);
